...
async def edit_or_delete_user(input_data: UpdateUser = Depends()):
    if input_data.action == 'EDIT':
        updates = {k: v for k, v in input_data.dict().items() if k not in ['id', 'action', 'param_to_delete'] and v is not None}
        update_user(input_data.id, updates)
